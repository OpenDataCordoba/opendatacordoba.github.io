...
getBaseData = function(){
	...
	$.when(provincias, formulas, listas, pasos, definitivas).done(function(provincias, formulas, listas, pasos, definitivas){
			...
			$.each(provs, function(prov_id, votos){
				...
				if (def_proyectado > 0){
					...
					var este = {'provincia': prov[prov_id].provincia,
								...
								'definitivas': definitiva_tot,
								'definitivas_porc': Math.round(votos_def_porc * 1000) / 1000,
								'definitivas_proyectado': Math.round(def_proyectado),
								'diferencia': Math.round(diff),
								'aporte_nacional': Math.round(diff_contra_nacional * 1000) / 1000 }
		...
		$.each(alidef, function(alianza, info){
			...
			// $th = '<tr><th>Provincia</th><th>Paso</th><th>Ahora</th><th>Proyectado</th><th>Diferencia</th><th>Var Nacional</th></tr>';
			$th = '<tr><th>Provincia</th><th>Paso</th><th>Ahora</th><th>Peso variación Nacional</th></tr>';
			$alianza_table.append($th);
			// total nacional
			nac = '<tr><td>Total Nacional</td><td>'+info[99].paso+' ('+info[99].paso_porc+' %)</td>';
			nac += '<td>'+info[99].definitivas+' ('+info[99].definitivas_porc+' %)</td>';
			// nac += '<td>'+info[99].definitivas_proyectado+'</td>';
			// nac += '<td>'+info[99].diferencia+'</td>';
			nac += '<td>'+info[99].aporte_nacional+'</td></tr>';
			$alianza_table.append(nac);
			
			for(var i=1; i<25; i++){
				pr = '<tr><td>'+info[i].provincia+'</td><td>'+info[i].paso+' ('+info[i].paso_porc+' %)</td>';
				pr += '<td>'+info[i].definitivas+' ('+info[i].definitivas_porc+' %)</td>';
				// pr += '<td>'+info[i].definitivas_proyectado+'</td>';
				//pr += '<td>'+info[i].diferencia+'</td>';
				pr += '<td>'+info[i].aporte_nacional+'</td>';
				pr += '</tr>';
